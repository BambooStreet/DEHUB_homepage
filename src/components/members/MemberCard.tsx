  ...
  return (
    <div className="p-6 bg-white rounded-xl border border-secondary-100 hover:shadow-md transition-shadow">
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary-100 to-primary-300 flex items-center justify-center text-primary-800 font-bold text-xl shrink-0">
          {member.name.charAt(0)}
        </div>
        <div className="min-w-0">
          <h3 className="font-semibold text-secondary-800">{member.name}</h3>
          <p className="text-sm text-secondary-400">{member.nameEn}</p>
      ...
      {member.research && member.research.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {member.research.map((tag) => (
            <span
              key={tag}
              className="px-2 py-0.5 text-xs bg-secondary-50 text-secondary-500 rounded-full"
            >
              {tag}
            </span>
          ))}
